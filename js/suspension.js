...
function getProgress(){
    ...
    $.post(url, {nombre_curso: 'Suspensión', token: sessionStorage.getItem('token')})
        .done(function(res){
            ...
            bindEvents();
            for (var i = 11; i > etapa+1; i--) { //TOTAL DE ETAPAS DEL CURSO (MODIFICAR)
                if(i!=0){
...
function initModals(){
    ...
    $('#n9').click(function(){
        $('#modal9').modal('open');
        launchProgressBar('#pb9', 32)
    })
    $('#n10').click(function(){
        $('#modal10').modal('open');
        launchProgressBar('#pb10', 5)
    })
    $('#n11').click(function(){
        $('#modal11').modal('open');
        launchProgressBar('#pb11', 60)
    })
...
function saveProgress(etapa, id){
    ...
    $.post(url, {token: sessionStorage.getItem('token'), etapa: etapa, curso: 1}) //En curso se indica el id del curso correspondiente
    .done(function(res){
        let response = JSON.parse(res);
        if(response.err){
            ...
            bindEvents();
            for (let i = 11; i > (parseInt(etapa)+1); i--) { //TOTAL DE ETAPAS DEL CURSO (MODIFICAR)
                if(i!=0){
                    $('#n'+i.toString()).off('click');
                }
            }
