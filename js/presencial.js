...
function getProgress(){
    ...
    $.post(url, {nombre_curso: 'Comportamiento Presencial', token: sessionStorage.getItem('token')})
        .done(function(res){
...
function saveProgress(etapa, id){
    ...
    $.post(url, {token: sessionStorage.getItem('token'), etapa: etapa, curso: 4}) //En curso se indica el id del curso correspondiente
    .done(function(res){
        let response = JSON.parse(res);
        if(response.err){
